...
export default function CannotStartSignatureStep({
  ...
  async function saveSignature() {
    ...
    try {
      ...
      const response = await fetch(
        '/api/compliance/cannot-start-signature',
        {
          method: 'PATCH',
          headers: {
      ...
      router.push(
        `/jobs/${jobId}/compliance/${recordId}?step=${nextStep}`
      )
    ...
    Clear Signature
      ...
      <div className="mt-6">
        <button
          ...
          onClick={() =>
            router.push(
              `/jobs/${jobId}/compliance/${recordId}?step=cannot_start_photos`
            )
          }
          disabled={saving}
  ...
  disabled={saving}
  className="w-full rounded-xl bg-orange-500 px-6 py-4 font-bold text-white transition hover:bg-orange-600 disabled:opacity-50"
>
